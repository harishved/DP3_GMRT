...
function execute(input)
  ...
  local base_threshold = 1.75 -- lower means more sensitive detection
  ...
  if exclude_original_flags then
    aoflagger.scale_invariant_rank_operator_masked(input, copy_of_input, 0.05, 0.05)
  else
    aoflagger.scale_invariant_rank_operator(input, 0.05, 0.05)
  end
